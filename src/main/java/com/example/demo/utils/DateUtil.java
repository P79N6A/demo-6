...
public class DateUtil {
    ...
    public static String getTomorrowDateNewFMTString(String sDate) {
        Date aDate = parseDateWebFormat(sDate);
        aDate = addDays(aDate, 1);
        return getWebDateString(aDate);
    }

    public static String getTomorrowDateNewFormatString(String sDate) {
    ...
    public static String getBeforeDayWebFormat(int days) {
        Date date = new Date(System.currentTimeMillis() - (ONE_DAY_MILL_SECONDS * days));
        DateFormat dateFormat = getNewDateFormat(webFormat);
    ...
    public static long getDiffNaturalDays(Date date1, Date date2) {
        ...
        try {
            diffDays =
                    (dateFormat.parse(dateStr1).getTime() - dateFormat.parse(dateStr2).getTime()) / (24 * 60 * 60 * 1000);
        } catch (ParseException e) {
            log.error("时间转换失败", e);
        }
